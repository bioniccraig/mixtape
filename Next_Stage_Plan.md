...
**Soft launch happened — and the data reprioritised everything (19 June).** ~82 signups in the first 5 days. Pulled the funnel live from Supabase: **only 23% of signups ever create a tape (19 of 82); 65% sign in and never build, not even a draft.** The bottleneck is **not** engineering, email, or auth (88% sign in fine) — it's **activation**. Actions taken: (1) shipped builder funnel analytics (`builder_opened` / `first_track_added` / `signin_prompt_shown`) to see *where* the 53 drop; (2) cold-start UX fixes — tape-first builder + "Add your first tracks" button, in-place email-code sign-in (replacing the momentum-killing magic-link round-trip), and a playable example tape on the splash. **Reprioritisation:** activation now leads; Stage B "harden for scale" (YouTube quota etc.) is deliberately *behind* fixing the leaky funnel — no point scaling traffic that bounces at 77%. Outstanding: Craig to add `{{ .Token }}` to the Supabase email templates (gates the new sign-in), then read the funnel before/after. Full detail in `CHANGELOG.md`.
